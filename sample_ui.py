"""
Sample Interview UI — Voice Recording + xAI Grok STT + JSON Save
"""
...
<html>
<head>
    <title>DV Interview - Voice</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { width: 700px; background: #16213e; border-radius: 16px; padding: 40px; box-shadow: 0 8px 32px rgba(0,0,0,0.3); }
        h1 { text-align: center; color: #e94560; margin-bottom: 30px; font-size: 24px; }
        .progress { text-align: center; color: #888; margin-bottom: 20px; font-size: 14px; }
        .question-box { background: #0f3460; padding: 20px; border-radius: 12px; margin-bottom: 25px; border-left: 4px solid #e94560; }
        .question-box p { font-size: 18px; line-height: 1.6; }
        .label { font-size: 12px; color: #e94560; text-transform: uppercase; margin-bottom: 8px; letter-spacing: 1px; }
        ...
        .record-btn { background: #e94560; color: white; }
        .record-btn:hover { background: #c23152; }
        .record-btn.recording { background: #ff0000; animation: pulse 1s infinite; }
        .stop-btn { background: #533483; color: white; }
        .stop-btn:hover { background: #3d2466; }
        .next-btn { background: #0f3460; color: white; border: 1px solid #e94560; }
        .next-btn:hover { background: #e94560; }
        .submit-btn { background: #2ecc71; color: white; }
        .submit-btn:hover { background: #27ae60; }
        button:disabled { opacity: 0.4; cursor: not-allowed; }
        .transcript-box { background: #0a1628; padding: 15px; border-radius: 8px; min-height: 60px; margin-bottom: 20px; border: 1px solid #333; }
        .transcript-box p { color: #aaa; font-style: italic; }
        .status { text-align: center; color: #e94560; font-size: 14px; margin-top: 15px; min-height: 20px; }
        .timer { text-align: center; font-size: 24px; color: #e94560; margin-bottom: 15px; font-family: monospace; }
        @keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.5; } }
        .done { text-align: center; padding: 40px; }
        .done h2 { color: #2ecc71; margin-bottom: 15px; }
    </style>
</head>
<body>
    <div class="container">
    ...
    <script>
        ...
        function stopRecording() {
            ...
            document.getElementById('status').textContent = 'Transcribing with xAI Grok...';
        }
        ...
        async function sendAudio() {
            ...
            try {
                ...
                if (data.error) {
                    ...
                    document.getElementById('status').textContent = 'Transcribed in ' + data.time + 's ✓';
            ...
            if (currentQuestion >= questions.length) {
                document.querySelector('.container').innerHTML = `
                    <div class="done">
                        ...
                        <p>All answers saved to interview_data.json</p>
                        <br>
...
def transcribe():
    ...
    try:
        ...
        if response.status_code != 200:
            return jsonify({"error": f"xAI API error: {response.text}"}), 500
        ...
        return jsonify({"text": transcript, "time": elapsed})
...
if __name__ == "__main__":
    print("\n=== Design Verification Interview UI ===")
    print(f"Questions: {len(QUESTIONS)}")
    print(f"STT: xAI Grok")
